...
const ChannelSidebar = () => {
    ...
    const [isInviteOpen, setIsInviteOpen] = useState(false)
    const [creating, setCreating] = useState(false)
    // Hook central que obtiene la lista de canales y expone loading/error
    const { channels, loading, error, load, createChannel } = useChannels(workspace_id)

    // Estado local que se sincroniza con `channels` y se pasa a ChannelList
    const [localChannels, setLocalChannels] = useState([])

    useEffect(() => {
        setLocalChannels(Array.isArray(channels) ? channels : [])
    }, [channels])
    ...
    async function handleCreateClick() {
        ...
        try {
            setCreating(true)
            await createChannel(trimmed) // createChannel del hook recarga la lista internamente
    ...
    // Cuando un canal es borrado, si era el activo navegamos al root del workspace
    const handleChannelDeleted = (deletedId) => {
        if (String(deletedId) === String(channel_id)) {
        ...
        <aside className="channel-sidebar" role="complementary" aria-label="Canales y mensajes directos">
            <div className='channel-sidebar-left'>
                <div className='channel-sidebar-icons'>
                    ...
                    <button className='sidebar-icon'>
                        <svg viewBox="0 0 20 20">
                            <path fill="currentColor" d="M14.5 10a1.75 1.75 0 1 1 3.5 0 1.75 1.75 0 0 1-3.5 0m-6.25 0a1.75 1.75 0 1 1 3.5 0 1.75 1.75 0 0 1-3.5 0A1.75 1.75 0 0 1 2 10"></path>
                        </svg>
                        <span>Más</span>
                    </button>
                </div>
            ...
            <div className='channel-sidebar-right'>
                ...
                {loading && <div className="channel-list-empty">Cargando...</div>}
                {localChannels && <ChannelList channel_list={localChannels || []} setChannels={setLocalChannels} onDeleted={handleChannelDeleted} />}
                {error && <div style={{ color: 'red' }}>Error al obtener la lista de canales</div>}
